...
def apply_rotary_pos_emb(x: jax.Array, sin: jax.Array, cos: jax.Array, scale: jax.Array) -> jax.Array:
    """
    Args:
    ...
    # Rotate every two
    rotated = jnp.stack([-x[:, :, 1::2], x[:, :, ::2]], axis=2).reshape(x.shape[0], x.shape[1], -1)
...
class XPos(nn.Module):
    ...
    def setup(self):
        self.scale = (jnp.arange(0 ,self.head_size, 2) + 0.4 * self.head_size )/(1.4*self.scale_base)
    ...
    def __call__(self, x: jax.Array, offset: int = 0, downscale=False) -> jax.Array:
        ...
        scale = self.scale ** (jnp.arange(min_pos, max_pos, 1)/self.scale_base)[:, None]
    ...
    def forward_reverse(self, x, offset=0, downscale=False) -> jax.Array:
        ...
        scale = self.scale ** (jnp.arange(min_pos, max_pos, 1)/self.scale_base)
        sin, cos = fixed_pos_embedding(scale.shape[0], scale.shape[1])
        if scale.shape[0] > length:
            scale = scale[-length:]
            sin = sin[-length:]
